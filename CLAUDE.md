...
- 2026-08-26 (#94): `UPLOAD_DIR` 은 `.env.production` 과 standalone 사본 **양쪽에** 절대 경로로
  ...
  201 → 파일이 `/var/lib/dicpress/uploads/` 에 생성 → 공개 URL 200
- 2026-08-25: `client_max_body_size 10m`(#61)과 `proxy_set_header Authorization "";`(#70)도
  이미 들어가 있다. 이 목록에 남아 있었으나 사실이 아니어서 지웠다

**확인할 때 무엇이 무엇을 증명하는지 구분할 것.** 이 목록이 틀리면 알아낼 방법이 없으므로
확인의 강도도 함께 적어 둔다.

| 방법 | 증명하는 것 | 증명하지 못하는 것 |
|---|---|---|
| `nginx -T \| grep …` | **돌고 있는** 설정 (디스크 파일이 아니다) | 앱이 어디에 쓰는지 |
| `grep -c '^UPLOAD_DIR='` | 줄이 있다 | 값이 절대 경로인지, 프로세스가 그 값으로 떴는지 |
| 새 디렉터리에 파일이 있음 | 아무것도 — `mv` 는 mtime 을 보존한다 | 앱이 쓴 것인지, 손으로 옮긴 것인지 |
| **업로드 왕복 (201 → 공개 200)** | 값·기동 반영·PM2 주입·서빙 **전부** | — |

앞의 셋은 전부 통과하는데 앱은 엉뚱한 데 쓰고 있는 상태가 성립한다. 왕복만이 그걸 배제한다.
