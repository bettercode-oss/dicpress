...
- 이메일은 **소문자로 정규화해 보내세요.** dicpress 가 받는 즉시 `trim` + `toLowerCase` 하므로
  ...
  로그 대조가 쉽습니다.
- **쓰기 요청은 기록됩니다** (#103). 권한 확인을 통과한 `POST`·`PATCH`·`DELETE` 는
  `AuditLog` 에 주체(`id`·이메일·역할)·경로·메서드·`source`(session/service)로 남습니다.
  콘솔이 부른 요청은 `source = "service"` 이고 **`X-Actor-Email` 이 가리킨 사람**으로
  기록됩니다 — 그 헤더를 세션에서 채워야 하는 이유가 하나 더 늘어난 셈입니다.
  읽기(GET)는 남기지 않습니다.
- **저장도 소문자입니다** (#105). `POST /api/admin/signup` 과 승인이 만드는 `User.email` 은
  모두 정규화를 거칩니다. 예전에는 읽기 쪽만 정규화해서, 대소문자가 섞여 저장되면
  **승인된 계정이 영구히 403** 을 받는 함정이 있었습니다. 정규화의 정의는
  `lib/email-address.ts` 한 곳에 있습니다.
...
| 404 | 대상 없음 |
| 409 | slug 중복, **이미 처리된 계정 신청**을 다시 승인·거절, `signup` 의 중복 신청 |
| 429 | **속도 제한** (#104). 인증 경로에만 걸린다 — 아래 참고 |
...
> **`POST /api/admin/signup` 의 409 는 계정 상태를 노출합니다.** "이미 신청된 이메일입니다"
> 와 "이미 승인된 이메일입니다" 를 화면에 그대로 옮기면, 로그인하지 않은 사람이 이메일
> 하나로 그 계정의 상태를 알아낼 수 있습니다. **접수 성공과 같은 문구로 뭉뚱그리세요**
> (콘솔은 그렇게 합니다 — `ahnyounghoe/admin#67`).
>
> 같은 이유로 인증 경로에는 nginx 속도 제한이 걸려 있습니다(#104). 한 IP 에서 분당 30건,
> 순간 20건까지는 그대로 통과하고 그 이상은 **429** 입니다. 사람의 로그인은 요청 두 개
> (options → verify)라 걸리지 않습니다. 대상은 `/api/auth/webauthn/*` 과 `/api/admin/signup`
> 이며, **콘솔은 루프백으로 부르므로 영향받지 않습니다.**
...
> dicpress 에는 없을 수 있고, 그게 정상입니다. 콘솔은 `X-Actor-Email` 을 **로그인 세션에서**
> 채우므로, 그 주소와 같은 `ACTIVE` 계정이 dicpress 에 있어야 합니다.
>
> **없다고 해서 콘솔 쪽에서 다른 주소로 바꿔 부르면 안 됩니다.** 그 순간 "어떤 콘솔
> 사용자가 어떤 이메일을 주장할 수 있는가" 가 콘솔 설정으로 새어 나가 신뢰 모델이
> 무너집니다. 할 일은 **dicpress 에 그 사람 계정을 만드는 것**이고, 계정이 없으면 403 이
> 나는 것이 정직한 실패입니다. (콘솔의 `DICPRESS_ACTOR_EMAIL` 은 로컬 admin DB 의 Owner
> 주소가 운영과 달라서 두는 **로컬 전용 탈출구**입니다. 운영 `.env` 에는 넣지 않습니다 —
> `ahnyounghoe/admin#51` 이 배포마다 그 부재를 확인합니다.)
>
